  ...
  const expectedStr =
    expectedResult !== null
      ? result === expectedResult
        ? ""
        : ` (Expected: ${expectedResult ? "ALLOWED" : "DENIED"})`
      : "";

  console.log(`${status}${expectedStr} - ${description}`);
  console.log(
    `  ${subjectNamespace}:${subject} -> ${relation} -> ${namespace}:${object}`,
  );
  console.log();
...
async function runTests() {
  ...
  try {
    ...
    console.log(
      "Note: These tests check permissions granted through group membership \n\t NOT Permissions granted directly to a Group",
    );
    console.log("-".repeat(70));
    console.log();
